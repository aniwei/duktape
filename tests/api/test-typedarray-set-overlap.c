...
static duk_ret_t test_basic_overlap(duk_context *ctx, void *udata) {
	...
	for (offset = 0; offset <= 8; offset++) {
		...
		duk_eval_string(ctx,
			...
			"    var b1 = new Uint8Array(Object(plain1).buffer);\n"
			"    var b2 = new Uint8Array(Object(plain2).buffer);\n"
...
static duk_ret_t test_expand_overlap(duk_context *ctx, void *udata) {
	...
	for (offset = 0; offset < 16; offset++) {  /* dst offset */
		...
		duk_eval_string(ctx,
			...
			"    var bsrc = new Uint8Array(Object(plain_src).buffer);\n"
			"    var bdst = new Uint32Array(Object(plain_dst).buffer);\n"
